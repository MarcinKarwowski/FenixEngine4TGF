...
{% block pageContent %}
    ...
    <button type="button" class="btn btn-primary btn-lg" onclick="openAjaxModal('/admin/wikipedia/edit/0', '#myModal', '#articleTemplate', function() { tinymce.init(tinyMCConfig); })" style="float: right; position: relative; top: -50px;">
        {{ t['articles-add'] }}
    </button>
    <div class="box">
        <div class="box-body">
            <table id="dataTableData" class="table table-bordered table-striped" cellspacing="0" width="100%" style="visibility: hidden;">
                ...
                ?>
                <tr>
                    <td style="font-weight: bold;">{{ data['deep'] }} {{ data['title'] }}</td>
                    <td><a onclick="openAjaxModal('/admin/wikipedia/edit/{{ data['id'] }}', '#myModal', '#articleTemplate', function() { tinymce.init(tinyMCConfig); })" class="btn btn-primary btn-sm" type="button">{{ t['edit'] }}</a></td>
                    <td><a href="/admin/wikipedia/delete/{{ data['id'] }}" class="btn btn-danger btn-sm" type="button">{{ t['delete'] }}</a></td>
                </tr>
                <?php } ?>
                </tbody>
            </table>
    ...
    <script id="articleTemplate" type="text/x-jsrender">
        <div class="art_edit_form">
            <form  action="/admin/wikipedia/save[#* if(data.obj.id) { #]/[#>obj.id#][#* } #]" method="post" class="" id="modalForm" name="modalForm">
                    <!-- text input -->
                    ...
                    <div class="form-group">
                      <label>Artykuł nadrzędny</label>
                      <select class="form-control" name="parent">
                      <option value="0">Brak</option>
                      <?php foreach ($treeClass -> retArr as $data) { ?>
                        <option value="{{ data['id'] }}" [#* if(data.obj.parent_id=={{ data['id'] }}) { #]selected="selected"[#* } #]>{{ data['deep'] }} {{ data['title'] }}</option>
                      <?php } ?>
                      </select>
                    </div>
                    <div class="form-group">
                        ...
                        <div style="position: relative;">
                            <textarea id="mytextarea" name="text">[#* if(data.obj.wikitext) { #][#>obj.wikitext.text#][#* } #] </textarea>
                        </div>
                    </div>
            </form>

...
{% block pageContent %}
    ...
    <button type="button" class="btn btn-primary btn-lg" onclick="openAjaxModal('/admin/articles/edit/0', '#myModal', '#articleTemplate', function() { tinymce.init(tinyMCConfig); })" style="float: right; position: relative; top: -50px;">
        {{ t['articles-add'] }}
    </button>
    <div class="box">
        <div class="box-body">
            <table id="dataTableData" class="table table-bordered table-striped" cellspacing="0" width="100%" style="visibility: hidden;">
                ...
                <tbody>
                <?php
                foreach (Main\Models\Articles::find() as $data) {
                ?>
                <tr>
                    <td style="font-weight: bold;"><?php if (isset($data -> level)) echo str_repeat('--', $data -> level); ?> {{ data.title }}</td>
                    <td>{{ data.getDate() }}</td>
                    <td><a onclick="openAjaxModal('/admin/articles/edit/{{ data.id }}', '#myModal', '#articleTemplate', function() { tinymce.init(tinyMCConfig); })" class="btn btn-primary btn-sm" type="button">{{ t['edit'] }}</a></td>
                    <td><a href="/admin/articles/delete/{{ data.id }}" class="btn btn-danger btn-sm" type="button">{{ t['delete'] }}</a></td>
                </tr>
                <?php } ?>
                </tbody>
            </table>
    ...
    <script id="articleTemplate" type="text/x-jsrender">
        <div class="art_edit_form">
            <form  action="/admin/articles/save[#* if(data.obj.id) { #]/[#>obj.id#][#* } #]" method="post" class="" id="modalForm" name="modalForm">
                    <!-- text input -->
                    ...
                    <div class="form-group">
                      ...
                      <select class="form-control" name="type">
                        <option value="NEWS" [#* if(data.obj.type=="NEWS") { #]selected="selected"[#* } #]>News</option>
                      </select>
                    </div>
                    <div class="form-group">
                        <label>Treść</label>
                        <div style="position: relative;">
                            <textarea id="mytextarea" name="text">[#* if(data.obj.textdata) { #][#>obj.textdata.text#][#* } #] </textarea>
                        </div>
                    </div>
            </form>

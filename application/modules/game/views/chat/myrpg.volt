    ...
    <ul id="content_menu">
        ...
        <div id="chat_chaters">
            <li class="chat_chater">
                ...
                -- <a href="/game/chat/myrpg/1/1">Archiwum</a>
            </li>
        </div>
    </ul>
{% endblock %}
            ...
            {% if page.items|length > 0 %}
                ...
                {% for rpg in page.items %}
                    <div msg="{{ rpg.id }}" class="chat_msg row_{{ rpg.id }}">
                        <div>{% if rpg.priv == 1 %}<i class="fa fa-lock"></i>{% else %}<i class="fa fa-unlock"></i>{% endif %} <a href="/game/chat/change/{{ rpg.id }}">{{ rpg.title }}</a> - <a href="/game/chat/create/{{ rpg.id }}">{{ t['edit'] }}</a></div>
                    </div>
                {% endfor %}
                <nav style="text-align: center;">
                    <ul class="pagination">

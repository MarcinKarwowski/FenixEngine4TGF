...
{% block menu %}
    <ul id="content_menu">
        <li class="content_menu_nagl profiles_nagl"><i class="fa fa-globe"></i> {{ t['notify-notify_nagl'] }}</li>
            ...
            <div>
                {% for msg in page.items %}
                    <div class="notify"><?php if (isset($nicons[$msg -> Notify -> type])) echo '<i class="fa '.$nicons[$msg -> Notify -> type].'"></i>'; else echo '<i class="fa fa-bolt"></i>'; ?>
                        <div class="notify_data">{{ msg.Notify.title }}</div>
                        <div class="notify_data" style="font-size: 6px; margin-top: 5px;">{{ msg.Notify.date }}</div>
